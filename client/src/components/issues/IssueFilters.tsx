...
import { CATEGORIES, STATUSES, categoryLabel, statusLabel } from '@/lib/issues'
    ...
    <>
      ...
      <div className="hidden md:flex items-center gap-2 flex-wrap">
        ...
        <div className="relative ml-auto">
          ...
          <Input type="search" placeholder={t('filters.searchPlaceholder')} value={search} onChange={e => onSearchChange(e.target.value)} className="pl-8 pr-7 h-8 w-full sm:w-56 text-sm" />
      ...
      <div className="md:hidden">
        <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={() => setMobileOpen(true)}>
          <SlidersHorizontal className="w-3.5 h-3.5" /> {t('filters.filtersButton')}
          {activeCount > 0 && (
        ...
        <Drawer open={mobileOpen} onOpenChange={setMobileOpen}>
          <DrawerContent>
            <DrawerHeader>
              <DrawerTitle>{t('filters.filtersTitle')}</DrawerTitle>
            </DrawerHeader>
            <div className="px-4 pb-4 space-y-3">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
                <Input type="search" placeholder={t('filters.searchPlaceholder')} value={search} onChange={e => onSearchChange(e.target.value)} className="pl-8 pr-7 h-9 w-full text-sm" />
                {search && <button onClick={() => onSearchChange('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"><X className="w-3.5 h-3.5" /></button>}
              </div>
            ...
            <DrawerFooter>
              <Button onClick={() => setMobileOpen(false)}>{t('common.done')}</Button>
            </DrawerFooter>
          </DrawerContent>
        </Drawer>
      </div>
