  ...
  return (
    <PageContainer width="full" className="h-full flex flex-col">
      <PageHeader
        ...
        subtitle="Archive of offboarded properties — read only"
        actions={
          <>
              ...
              paged.map((p: any) => (
                <tr key={p.id} data-testid={`row-previous-${p.id}`} className="border-b border-border/50 hover:bg-muted/20 transition-colors opacity-80">
                  ...
                  <td className="py-2 px-3">
                    <span className="text-xs font-medium px-1.5 py-0.5 rounded border bg-muted text-muted-foreground border-border">
                      Offboarded
                    </span>
                  </td>
