...
// Droppable column
function StageColumn({ stage, properties, onCardClick }: {
  stage: any
  properties: any[]
  ...
  return (
    <div className="flex flex-col min-w-[220px] max-w-[240px]">
      <div className="flex items-center gap-2 mb-2 px-1">
        ...
        <span className="text-xs text-muted-foreground ml-auto tabular-nums">{properties.length}</span>
      </div>
      <div
        ref={setNodeRef}
        data-testid={`column-${stage.name}`}
        className={`flex-1 min-h-[200px] rounded-lg p-2 space-y-2 transition-colors ${isOver ? 'bg-primary/5 ring-1 ring-primary/30' : 'bg-muted/40'}`}
      >
        {properties.map(p => (
          <DraggableCard key={p.id} property={p} stageName={stage.name} stageColor={color} onClick={() => onCardClick(p)} />
        ))}
      </div>
    </div>
  )
...
// Draggable card
function DraggableCard({ property, stageName, stageColor, onClick }: {
  property: any; stageName: string; stageColor: string; onClick: () => void
}) {
