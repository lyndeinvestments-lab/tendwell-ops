...
// Admin-side change of an owner's login email (Settings → Owners). Runs
// server-side at /api/owners/admin-change-email (service role; admin-gated).
export async function adminChangeOwnerEmail(
  ownerId: string,
  newEmail: string,
  ...
  if (!token) return { ok: false, error: 'Not signed in' }
  try {
    const res = await fetch('/api/owners/admin-change-email', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ ownerId, newEmail }),
    })
    const data = await res.json().catch(() => ({}))
    if (!res.ok) return { ok: false, error: data.error || `Failed (${res.status})` }
    return { ok: true }
