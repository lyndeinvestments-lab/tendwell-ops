...
- **Trellis task deep-links (2026-07-10, branch `claude/trellis-task-links`):** every row on `/trellis-tasks` links to the task in the Trellis web app. Helper `trellisTaskUrl(id)` → `https://app.trellistech.com/tasks/<trellis_task_id>` (same host for both workspaces; UUID path, no workspace slug — confirmed with Jordan). Desktop: the property name is a link + a trailing external-link icon column. Mobile: the whole card is the link (external-link icon by the property name). No DB/schema change.
