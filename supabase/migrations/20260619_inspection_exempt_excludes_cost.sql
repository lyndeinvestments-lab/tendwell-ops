-- 20260619_inspection_exempt_excludes_cost.sql
-- When a property is inspection-exempt (exempt_from_inspections = true), the
-- inspection charge must be removed from its financials. We make the
-- recalc_property_formulas() trigger compute an *effective* inspection cost
-- (0 when exempt) and use that both for the per-property `inspection_cost`
-- column and inside `total_estimated_cost`. Setting inspection_cost to 0 for
-- exempt props also makes the Master List (cost-tracking) and Property Detail
-- modal breakdowns correct, since they read the per-row inspection_cost.
--
-- Idempotent: CREATE OR REPLACE + a backfill that re-fires the BEFORE trigger
-- for exempt rows so their stored inspection_cost/total are recomputed.
...
BEGIN
  ...
  -- Inspection-exempt properties carry no inspection charge.
...
-- Backfill: re-fire the BEFORE trigger for exempt rows so their stored
-- inspection_cost flips to 0 and total_estimated_cost/profit recompute.
UPDATE public.properties
SET updated_at = now()
WHERE COALESCE(exempt_from_inspections, false) = true;
