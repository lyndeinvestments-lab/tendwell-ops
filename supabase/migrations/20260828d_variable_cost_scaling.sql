-- Per-property variable costs: scale the sheet formula to QBO actuals.
--
-- Validation against July 2026 found the per-clean laundry+consumables
-- formula totals $63k against $48k of actual QBO laundry+supplies spend
-- (~31% high). The formula's RELATIVE weights are right (bigger cabins cost
-- more), so for months QBO has synced, each property's formula share is
-- scaled so the month's variable total matches QBO's actual
-- Laundry + Cleaning Supplies + Supplies Expense. Months without QBO data
-- keep the raw formula. Cleaner-pay attribution needed no change
-- (attributed $160.5k vs QBO contractor pay $158.7k in July — within 1.2%).
...
formula_totals as (
  -- Month-wide total of the raw per-clean formula, for actual-cost scaling.
  select c.month,
...
select
  ...
  -- Formula weights × cleans, scaled to the month's actual QBO
  -- laundry+supplies spend when QBO has the month; raw formula otherwise.
  round((
