-- Current-week coherence: fill the uninvoiced tail at sheet rates.
--
-- Cleans are counted through today, but invoicing (and QBO entries) only
-- cover through the newest invoice period (weekly, so up to ~6 days behind).
-- Before this, an in-progress month carried FULL costs (variable + overhead
-- scale with cleans) but only PARTIAL revenue/pay — profit sagged all week
-- and the "vs Expected" gap flagged cleans nobody has billed yet.
--
-- Now: cleans dated after the invoiced-through date (max period_end across
-- live runs) contribute revenue and cleaner pay at sheet rates (deep cleans
-- at the deep rate) ON TOP of the actuals, and the same tail is included in
-- expected revenue, so the gap column only measures the window an invoice
-- actually covered.
...
tail as (
  ...
  from (
    ...
    select p.id,
           ...
           count(*),
           count(*) filter (where t.title ilike '%deep%')
